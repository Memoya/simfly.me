/**
 * Country mascot landmark data
 * Maps ISO code → { country name (for prompt), landmark (for prompt), landmark description }
 *
 * Used to generate consistent AI image prompts per country:
...
export interface MascotLandmarkEntry {
  country: string;
  landmark: string;
}

export const mascotLandmarks: Record<string, MascotLandmarkEntry> = {
  // Europe
...
/**
 * Get mascot landmark data for a given ISO code
 */
export function getMascotLandmark(iso: string | undefined): MascotLandmarkEntry | null {
  if (!iso) return null;
  return mascotLandmarks[iso.toLowerCase()] ?? null;
}

/**
 * Generate an AI image prompt for a country mascot
 * Paste this into Midjourney, DALL-E 3, Stable Diffusion, etc.
 */
export function generateMascotPrompt(iso: string): string | null {
  const entry = getMascotLandmark(iso);
  if (!entry) return null;

  return (
    `3D mascot shaped like a SIM card, modern playful style, ` +
    `country: ${entry.country}, wearing subtle flag-inspired colors, ` +
    `holding or standing near a minimal travel icon of ${entry.landmark}, ` +
    `clean soft gradient background, high detail 3D render, ` +
    `brand-consistent, glossy material, front-facing, centered composition`
  );
}
